...
#
################################
##r = prueba.reconstruyeKNN('x_i',h,m,tau,k)
...
def vecinosCercanos(objeto:object,nameCol:str,h:int,m:int,tau:int,k:int,S:str = 'E'):      
    ...
    nameCol : str
        Nombre de la columna a reconstruir
    h : int
        Valor del hueco
    m : int
    ...
    """    
    #Variables
    data = objeto.verificaDatos('x_i',S)                                #Obtenemos los datos que pertenezcan al conjunto definido
    ...
    indicesHuecos = np.asarray(objeto.contar(nameCol,'h')[0]) #Obtengo el vector de conteo de posiciones inciales y tamaño de huecos
    ...
    for elemento in indicesHuecos:                      #Avanzo en los huecos de tamaño 1
        v1 = data[int(elemento-n):int(elemento+n+h)].reset_index(drop=True)   #Obtengo el primer vector a comparar
        #print(v1)
        modulo = 0       
        for i in range(n,(len(data)-(h+n))):            #Avanza sobre los datos, dejando espacio para que el vector "entre" y "salga" sin salir del vector de datos
            ...
            #print(v2)
            if objeto.validaIndices(v2,indices):               #Verifica que el vector que almacena el primer vector a comparar cumpla con las condiciones necesarias y que además no se compare consímismo                   
